...
import com.vira.echsan.ui.activity.hajiumroh.PaketHajiDetailActivity
import kotlinx.android.synthetic.main.segment_paket.view.*

class PaketHajiAdapter(private val context: Context) :
    RecyclerView.Adapter<PaketHajiAdapter.ViewHolder>() {
        ...
        fun bind(model: ResUmroh, position: Int, ctx: Context) {
            ...
            itemView.containerBeli.setOnClickListener {
                if (session.getString(session.TOKEN).equals("", true)) {
                    ...
                    val intent = Intent(ctx, PaketHajiDetailActivity::class.java)
