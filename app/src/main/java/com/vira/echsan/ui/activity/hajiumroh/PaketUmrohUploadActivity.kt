...
class PaketUmrohUploadActivity : AppCompatActivity(), View.OnClickListener, PermissionListener {
    ...
    private fun doBeliUmroh(id_paket: String) {
        showLoading()
        val apiService = ApiClient.getClient().create(ApiRequest::class.java)
        apiService
        ...
        EasyImage.handleActivityResult(requestCode, resultCode, data, this, object : DefaultCallback() {
            ...
            override fun onImagePicked(imageFile: File, source: EasyImage.ImageSource, type: Int) {
                if (imageFile.length() > 5242880) {
                    Toast.makeText(this@PaketUmrohUploadActivity, "Ukuran gambar terlalu besar!", Toast.LENGTH_LONG).show()
                } else {
                    if (imageFile.length() > 5242880) {
                        Toast.makeText(this@PaketUmrohUploadActivity, "Ukuran gambar terlalu besar!", Toast.LENGTH_LONG).show()
                    } else {
            ...
            override fun onCanceled(source: EasyImage.ImageSource?, type: Int) {
                ...
                if (source == EasyImage.ImageSource.CAMERA) {
                    val photoFile = EasyImage.lastlyTakenButCanceledPhoto(this@PaketUmrohUploadActivity)
                    photoFile?.delete()
                }
            }
        })
