...
import kotlinx.android.synthetic.main.activity_paket_umroh.*
import kotlinx.android.synthetic.main.xml_recycler.*
import kotlinx.android.synthetic.main.xml_toolbar.*
import retrofit2.HttpException
import java.util.*

class PaketUmrohActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun initRecycler() {
        ...
        scrollListener =
            object : EndlessRecyclerViewScrollListener(recycler.getLayoutManager() as LinearLayoutManager) {
                override fun onLoadMore(page: Int, totalItemsCount: Int, view: RecyclerView) {

                    getUmroh((times * 20).toString(), "20", "", "DESC")
                }
            }
        recycler.addOnScrollListener(scrollListener)
    }

    private fun getUmroh(start: String, length: String, search: String, sorting: String) {
        showLoading()
        val apiService = ApiClient.getClient().create(ApiRequest::class.java)
        apiService
            .umroh("list", start, length, search, sorting)
