...
import com.vira.echsan.adapter.FilterDurasiAdapter
import com.vira.echsan.adapter.FilterLokasiAdapter
...
class PaketUmrohFilterDialog : DialogFragment() {
    ...
    fun initRecyclerView(){
        val listDurasi = ArrayList<FilterDurasiModel>()
        for (i in 1..20) {
            listDurasi.add(FilterDurasiModel("$i hari"))
        }

        val listLokasi = ArrayList<FilterLokasiModel>()
        for (i in 1..20) {
            listLokasi.add(FilterLokasiModel("Lokasi $i"))
        }

        val durasiAdapter = FilterDurasiAdapter(activity!!, listDurasi)
        val lokasiAdapter = FilterLokasiAdapter(activity!!, listLokasi)
        ...
        rv_filter_durasi.adapter = durasiAdapter
        rv_filter_lokasi.adapter = lokasiAdapter
